...
# run as sage ./Find_solutions_Step6Table.sage CMfieldnumber p (q)
# e.g. sage ./Find_solutions_Step6Table.sage 2 3 runs for CM field 2, p = 3, all max orders in the table BasesForPrimesTable.csv
...
# e.g. sage ./Find_solutions_Step6Table.sage 26666 17 3 runs for CM field 26666, p = 17, q = 3, all max orders in the table BasesForPrimesTable.csv
#-------------------------------------------------------------------------------------------------------------------------------------------
# Step 6: Check if a solution in terms of Norms and Traces (Step 4) leads to a solution in terms of elements di.
# Read solutions file from Step 4: solutions have the form [x, dOVERn, a, cOVERn, b, gamma, n, Nd1, Nd2, Nd3, Trd1d2, Trd1d3, Trd2d3, Dd1d2, Dd1d3, Dd2d3, Dd1d2d3]
# Read di's from file from Step 5: all di's with trace 0 and norms Nd1, Nd2, Nd3.
# For each solution from Step 4, check if the di's from Step 5, with norms Nd1, Nd2, Nd3, can give traces with values Trd1d2, Trd1d3, Trd2d3.
#-------------------------------------------------------------------------------------------------------------------------------------------
# Files created:
# For each maximal order, a file CMfield"+CMfieldnumber.str()+"p"+p.str()+"MaxOrderBasis_"+O+Solutions_for_di_Step6.csv"
# Example: CMfield1p7MaxOrderBasis_1_+i_+frac{1}{2}i+frac{1}{2}k_frac{1}{2}+frac{1}{2}j_Solutions_for_di_Step6.csv
#-------------------------------------------------------------------------------------------------------------------------------------------
