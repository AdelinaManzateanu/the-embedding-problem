...
# run as sage ./Find_solutions_Step5xTable.sage CMfieldnumber p (q)
# e.g. sage ./Find_solutions_Step5xTable.sage 34 11 runs for CM field 34, p = 11, all max orders in the table BasesForPrimesTable.csv
...
# e.g. sage ./Find_solutions_Step5xTable.sage 34 17 3 runs for CM field 34, p = 17, q = 3, all max orders in the table BasesForPrimesTable.csv
#-------------------------------------------------------------------------------------------------------------------------------------------
...
# Save all norms and find all x_i in O with Tr = 0 and those norms
#-------------------------------------------------------------------------------------------------------------------------------------------
# Files created:
# For each max order, a file "CMfield"+CMfieldnumber+"p"+p+"MaxOrderBasis_"+O+x.csv" is created.
# example: CMfield34p11MaxOrderBasis_1_+i_+frac{1}{2}i+frac{1}{2}k_frac{1}{2}+frac{1}{2}j_x.csv
#-------------------------------------------------------------------------------------------------------------------------------------------
...
# Read solutions: x, dOVERn, a, cOVERn, b, gamma, n, Nx1, Nx2, Nx3, Trx1x2, Trx1x3, Trx2x3, Dx1x2, Dx1x3, Dx2x3, Dx1x2x3
...
## Each row if of the form [x, dOVERn, a, cOVERn, b, gamma, n, Nx1, Nx2, Nx3, Trx1x2, Trx1x3, Trx2x3, Dx1x2, Dx1x3, Dx2x3, Dx1x2x3]
for row in reader:
	x = QQ(row["x"])
	...
	gamma = QQ(row["gamma"])
	n = QQ(row["n"])
	Nx1 = eval(preparse(row["Nx1"]))
	Nx2 = eval(preparse(row["Nx2"]))
	Nx3 = eval(preparse(row["Nx3"]))
	Trx1x2 = eval(preparse(row["Trx1x2"]))
	Trx1x3 = eval(preparse(row["Trx1x3"]))
	Trx2x3 = eval(preparse(row["Trx2x3"]))
	Dx1x2 = eval(preparse(row["Dx1x2"]))
	Dx1x3 = eval(preparse(row["Dx1x3"]))
	Dx2x3 = eval(preparse(row["Dx2x3"]))
	Dx1x2x3 = eval(preparse(row["Dx1x2x3"]))
	if Nx1 not in all_norms:
		all_norms += [Nx1]
	if Nx2 not in all_norms:
		all_norms += [Nx2]
	if Nx3 not in all_norms:
		all_norms += [Nx3]
...
for base in all_bases:
	...
	csvwriter.writerow(('N', 'x with trace 0 and norm N'))
