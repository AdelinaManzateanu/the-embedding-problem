...
# run as sage ./Find_solutions_Step93v2Table.sage CMfieldnumber p (q)
# e.g. sage ./Find_solutions_Step93v2Table.sage 2 3 runs for CM field 2, p = 3, all max orders in the table BasesForPrimesTable.csv
...
# e.g. sage ./Find_solutions_Step93v2Table.sage 26666 17 3 runs for CM field 26666, p = 17, q = 3, all max orders in the table BasesForPrimesTable.csv
#-------------------------------------------------------------------------------------------------------------------------------------------
# Step 9:
# Read solutions x,dOVERn,a,cOVERn,b,gamma,n,x1,x2,x3,x4,x5,x6,x7,x8,x9,Dx1x2,Dx1x3,Dx2x3,Dx1x2x3 from Step 8
# Check that if they are equivalent due to Proposition 4.2
#-------------------------------------------------------------------------------------------------------------------------------------------
# Files created:
# One file CMfield"+CMfieldnumber.str()+"p"+p.str()+"MaxOrderBasis_"+O+Solutions_for_di_Step93v2.txt" with equivalence classes of solutions in the form [x, dOVERn, a, cOVERn, b, gamma, n, x1, x2, x3, x4, x5, x6, x7, x8, x9, Dx1x2, Dx1x3, Dx2x3, Dx1x2x3]
# Example: CMfield1p7MaxOrderBasis_1_+i_+frac{1}{2}i+frac{1}{2}k_frac{1}{2}+frac{1}{2}j_Solutions_for_di_Step93v2.txt
#-------------------------------------------------------------------------------------------------------------------------------------------
...
for base in all_bases:
	...
	print ("reading solutions file from Step 8...")
	sys.stdout.flush()
	tread = time()
	...
	filename1 = filename1+"Solutions_for_di_Step8v2.csv"
	...
	filename2 = filename2+"Solutions_for_di_Step93v2.txt"
	...
	print('There are at most ', len(final), 'equivalence classes of solutions.')
	file2.close()
